...
from rsl_rl.modules import ActorCriticSwAV
from rsl_rl.storage import RolloutStorageNP3O
from  rsl_rl.utils import unpad_trajectories

class NP3O:
    actor_critic:ActorCriticSwAV
    def __init__(self,
                 actor_critic,
                 depth_encoder,
                 depth_encoder_paras,
       ...
       #注释
        ...
        self.transition = RolloutStorageNP3O.Transition()
    ...
    def init_storage(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, action_shape,cost_shape,cost_d_values):
        self.storage = RolloutStorageNP3O(num_envs, num_transitions_per_env, actor_obs_shape,  critic_obs_shape, action_shape,cost_shape,cost_d_values,self.device)
    ...
    def compute_cost_surrogate_loss(self,actions_log_prob_batch,old_actions_log_prob_batch,cost_advantages_batch):
        ...
        # surrogate_loss = torch.max(surrogate, surrogate_clipped).mean(dim=[0,1])
        return surrogate_loss
    ...
    def update_k_value(self,i):
        ...
        # self.k_value = torch.ones_like(self.k_value)
        return self.k_value
    ...
    def compute_viol(self,actions_log_prob_batch,old_actions_log_prob_batch,cost_advantages_batch,cost_volation_batch):
        ...
        # cost_loss = cost_surrogate_loss +cost_volation_batch# [，cost_shape]
        # return torch.sum(self.k_value * F.relu(cost_loss), dim=-1).mean()
